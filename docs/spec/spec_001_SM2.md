...
| --- | --- | --- |
| 曲线与点 | `P256Sm2`、`IsOnCurve`、`Add`、`Double`、`ScalarMult`、`ScalarBaseMult` | `sm2_p256()`、`Point` 上的方法 |
...
这条减式与 NIST P-256 一致，因此可以「直接抄」P-256 的快速约简路径；本规范采用 4 个 64-bit 字的小端字序（[low..high]），所有函数都对 4 字数组原地操作；字节 I/O 走大端。

> 与 gmsm 用 9 个 29/28-bit 字的内部表示相比，本规范用 4 × 64-bit 是为了可读性与 MoonBit 的 `UInt64` 路径；性能上比 gmsm 略差但足够覆盖签名/加解密/密钥协商三种工作负载。
...
| `bn_to_bytes(r) -> Bytes` | | 4 字 → 32 字节大端 |
...
| `bn_sqr(r, a)` | 同上但共用 a |
| `bn_mod_p(r)` | 256→256 快速约简（用 P-256 风格递推，公式见下） |
| `bn_modmul_p(r, a, b)` | 调 `bn_mul` 后 `bn_mod_p` |
| `bn_modsqr_p(r, a)` | 同上 |
| `bn_modinv_p(r, a)` | Fermat：`a^(p-2) mod p`（平方-乘 256 轮）；后续可换为 `addchain` |
| `bn_mod_n(r, a)` | 模曲线阶 n（用 8 字「n`+0 形式`」简单减法；n < 2^256 仅有 1 个借位，与 P-256 一样可走快速路径） |
| `bn_rand_k(random) -> FA[U64]` | 1 ≤ k ≤ n-2（用 MoonBit `random` 接口；如果传 `None` 则用 `Rand::chacha8` 或 `@random.Rand`） |

P-256 快速约简（参考 [`go/src/crypto/elliptic/p256.go`](https://github.com/golang/go/blob/master/src/crypto/elliptic/p256.go)）—— 把 `s0..s7`（8 个 64-bit 字）按下式拆为 4 字（`T[0..3]`）：

```
T[0] = s0 + s4 + s5 - s6 - s7   (mod 2^64)
T[1] = s1 + s6 - s4 - s7 + s5   (mod 2^64)
T[2] = s2 + s7 - s5             (mod 2^64)
T[3] = s3 + s6 + s4 - s5        (mod 2^64)
```

然后做条件减 `p` 3 次处理负值；最后再串行修正 3 个条件减。这条路径与 gmsm 的 9×29-bit 路径数值上同源，正确性已被标准 KAT 覆盖。

阶 `n` = `FFFFFFFE FFFFFFFF FFFFFFFF FFFFFFFF 7203DF6B 21C6052B 53BBF409 39D54123` < 2^256，模 n 可用经典 256-bit schoolbook + Barrett / Montgomery 简单减法链（本规范使用 gmsm 同款的「算到 512-bit 再用 `n` 减几次」朴素方案）。
...
- `za(curve, pub, uid) -> Bytes`：`H256(ENTL ‖ uid ‖ a ‖ b ‖ Gx ‖ Gy ‖ xA ‖ yA)`；
- `msg_hash(za, msg) -> FixedArray[UInt64]`：`SM3(za ‖ msg)` 转 256-bit 整数；
- `kdf(length, parts...) -> Bytes`：循环 `(length+31)/32` 次 `SM3(parts ‖ ct)` 拼出 `length` 字节；若全 0 则返回 `ok=false`（与 gmsm 同）；
- `ke_x_hat(x) -> FixedArray[UInt64]`：置低 128-bit 最高位为 0，置 128-bit 段最高位为 0；然后加 `2^127`（gmsm 用 `2^w` 偏移 w=127；本规范复刻）。
- `int_to_bytes_be(n) -> Bytes`：32-bit → 4 字节大端（KDF 计数器使用）。
...
// 密钥对
pub fn generate_key(random : @random.Rand?) -> (Bytes, SM2PublicKey)
pub fn derive_public_key(private_key : Bytes) -> SM2PublicKey
...
pub fn key_exchange_a(
  ...
  priA : Bytes, pubB : SM2PublicKey,
  rpriA : Bytes, rpubB : SM2PublicKey,
) -> (Bytes, Bytes, Bytes) raise Sm2Error
pub fn key_exchange_b(...) -> (Bytes, Bytes, Bytes) raise Sm2Error
...
| 仿射点字节表示 | 04 ‖ X ‖ Y | 65 B | 与 gmsm `Encrypt/Decrypt` 的密文首字节一致 |
| 压缩公钥 | 02/03 ‖ X | 33 B | 高 1 位表示 `y` 奇偶 |
| 签名 ASN.1 | `SEQUENCE { r INTEGER, s INTEGER }` DER | 可变 | 与 gmsm `SignDigitToSignData` 一致 |
| 密文 ASN.1 | `SEQUENCE { x INTEGER, y INTEGER, hash OCTET STRING, cipherText OCTET STRING }` | 可变 | 与 gmsm `CipherMarshal` 一致 |
| 密文布局 `C1C3C2` | 04 ‖ C1(64) ‖ C3(32) ‖ C2 | 1+64+32+klen | |
| 密文布局 `C1C2C3` | 04 ‖ C1(64) ‖ C2 ‖ C3(32) | 1+64+klen+32 | |
...
- `key_exchange_b(16, ida, idb, db, da_pub, rb, ra_pub).k == expk`
- `key_exchange_a(16, ida, idb, da, db_pub, ra, rb_pub).k == expk`
- 双方 S1/Sb 与 Sa/S2 互等（按 gmsm 注释 `S1=Sb`、`Sa=S2`）。

### 4.4 签名的确定性 KAT（来自 `sm2.go:43-47` 的私钥与 ZA）

为支持「固定随机源 → 固定签名」的可重复测试，引入 gmsm 同款 `zeroReader{}`（`sm2.go:667-672`）：
...
把 `zeroReader` 作为 `random` 参数注入 `sm2_sign` / `sm2_encrypt` 后，可得到确定性输出；本规范要求在测试中以「固定私钥 + zero reader + 固定 msg + 固定 uid」派生 KAT（具体签名值将由 gmsm 的 Go 实现首先生成，本规范留「在 gmsm CI 中抓取并粘贴到本文件作为 `expected` 字段」的占位段；首版可暂时只校验「同输入同输出 + 验证通过」）。

### 4.5 加密布局 KAT（来自 `sm2_test.go:48-58` 的 `TestSm2`）

```text
msg = "123456"
d  = 随机生成（生产 KAT 时由 gmsm CLI 给出，本规范用 zeroReader 派生确定性密文）
expected_layout_C1C2C3 = 04 ‖ X(32) ‖ Y(32) ‖ "123456" ‖ SM3(x2‖"123456"‖y2)  (掩码后)
```

期望：

- `len(cipher) == 1 + 64 + len(msg) + 32`；
- 第 1 字节 = `0x04`；
- 解密回原 `msg`；
...
1. **密钥对**：`generate_key` 返回 32 字节私钥，且对应公钥 `is_on_curve` 为真，`derive_public_key(d)` 与 `generate_key(d, ...)` 的公钥一致。
2. **签名 round-trip**：
   - 用 `generate_key` 产生密钥；
   - 固定 `uid = b"1234567812345678"`；
   - `sm2_sign(d, msg, uid, ZeroReader{})` 得 `(r, s)`；
   - 断言 `sm2_verify(P, msg, uid, r, s) == true`；
   - 翻转 `r` 一位后断言 `verify == false`。
3. **签名 DER round-trip**：
   - `sm2_sign_der` 得 ASN.1；
   - `sm2_verify_der` 通过；
   - 翻转 ASN.1 中 r 的最后一字节后断言 `verify == false`。
4. **签名 KAT**：以本规范 §4.3 中 `da` 作为私钥，固定 `msg=b"message digest"`、`uid=default_uid`、`random=ZeroReader{}`，期望 `(r, s)` 的 16 进制值（数值由「在 gmsm Go 测试中用同样输入跑出后贴入」；占位字段名 `expected_r_hex` / `expected_s_hex`）。
5. **加密 round-trip**：
   - `sm2_encrypt(P, msg, ZeroReader{}, C1C3C2)` 长度 = 1+64+32+len(msg)；
   - `sm2_decrypt(d, cipher, C1C3C2) == msg`；
   - C1C2C3 同上；
   - 翻转 C3 任一字节后 `sm2_decrypt` 抛 `Sm2Error::InvalidCipher`。
6. **加密 KAT**：
   - `da` 作私钥（§4.3）；
   - `msg = b"123456"`、`random = ZeroReader{}`、`mode = C1C3C2`；
   - 期望密文 hex 字符串（与签名的「贴入」机制相同）。
7. **ASN.1 round-trip**：`cipher_marshal` ↔ `cipher_unmarshal` 互逆。
8. **密钥协商 KAT**（核心）：完整复刻 `TestKEB2`。
   - 输入：`ida, idb, da, db, ra, rb, klen=16`（§4.3）；
   - 期望：`k1 == k2 == expk`，`Sb == S1`、`S2 == Sa`；
   - 同时验证 `KeyExchangeA` 失败场景：`pubB` 传一个非法点 → `Sm2Error::NotOnCurve`。
9. **压缩 / 解压**：
...
10. **跨实现互操作（手工 / CI 可选）**：
    - 在 Go 端跑 `gmsm/sm2`，用同一私钥 + zeroReader 产出 `expected_r` / `expected_s` / `expected_cipher`，作为 fixture 文件落到 `sm2/testdata/`；
    - MoonBit 测试读 fixture 验证一致。
    - 该路径不在本规范硬性要求中，但 `docs/SM2-SPEC.md` 留出 fixture 文件命名约定（见 §7）。
...
| 里程碑 | 文件 | 验收 |
| --- | --- | --- |
| M1 大数层 | `sm2/bn.mbt` | `moon check`、`bn_*` 全部白盒通过 |
| M2 点运算 | `sm2/curve.mbt` | `point_double/add` 与硬编码 KAT 比对通过 |
| M3 密钥对 | `sm2/sm2.mbt` 替换 `generate_key`/`derive_public_key`/`is_on_curve` | `generate_key` 与 gmsm `GenerateKey` 字节级一致 |
| M4 签名 | `sm2/sm2.mbt` `sm2_sign`/`sm2_verify` | 黑盒 round-trip + gmsm KAT 一致 |
| M5 加解密 | `sm2/sm2.mbt` `sm2_encrypt`/`sm2_decrypt` | 黑盒 round-trip + gmsm KAT 一致 |
| M6 协商 | `sm2/sm2.mbt` `key_exchange_a/b` | `TestKEB2` KAT 全通过 |
| M7 ASN.1 + 压缩 | `sm2/sm2.mbt` | DER round-trip + 压缩 round-trip |
...
1. **UInt64 性能**：4×64-bit 路径在 MoonBit 后端（wasm-gc / native）下未做过 benchmark；如实测过慢，再切换到 8×32-bit 或 9×29-bit（与 gmsm 同款）。
...
4. **互操作 fixture**：若 gmsm Go 测试不可在 CI 中运行，KAT 需在初始化实现时一次性离线生成并提交。
